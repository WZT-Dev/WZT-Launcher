...
public class GameTasks extends Thread {
	...
	private final ProfilesManager.LauncherProfile profile;
	...
	public GameTasks(final ProfilesManager.LauncherProfile profile, final JProgressBar downloadProgress) {
	...
	public void run() {
		...
		try {
			...
			final UsersManager.User user = UsersManager.getUserByID(profile.user);
			...
			command.addAll(getMinecraftArgs(game, user == null ? new UsersManager.User("Player", UUID.nameUUIDFromBytes(("OfflinePlayer:Player").getBytes(Charset.forName("UTF-8"))).toString().replace("-", ""), "Player", false, "0", new ArrayList<AuthUser.Property>()) : user, gson, assetsDir, assetsObjectsDir));
	...
	private final List<String> getMinecraftArgs(final Game game, final UsersManager.User user, final Gson gson, final File assetsDir, final File assetsObjectsDir) {
		...
		for(final AuthUser.Property property : user.properties) {
	...
	public class Library {
		...
		public final boolean isDisallowed(final SystemManager.OS os) {
			if(rules != null) {
